...
access_auth = JwtAccessBearer(secret_key=settings.authjwt_secret_key,
                       auto_error=True,
...
class AuthMiddleware(BaseHTTPMiddleware):
  ...
  async def dispatch(self, request, call_next):
    ...
    try:
      payload = access_auth.jwt_backend.decode(request.headers['Authorization'].split('Bearer ')[1], access_auth.secret_key)
      if payload['exp'] < time.time():
        return ORJSONResponse(status_code=401, content={'detail': 'Token expired'})
    except:
...
async def get_user(sub: JwtAuthorizationCredentials = Security(access_auth)) -> User | None:
...
async def user_login(body: AuthLoginBody) -> AuthLoginResponse:
  user = await engine.find_one(User, (User.email == body.emailUsernamePhone) | (User.username == body.emailUsernamePhone) | (User.phone == body.emailUsernamePhone))
  if user and bcrypt.checkpw(body.password.encode('utf-8'), user.bcryptPassword.encode('utf-8')):
    sub = {
      "id": str(user.id),
    ...
    return ORJSONResponse({
      ...
      'refresh_token_expire_time': math.floor(time.time()) + REFRESH_EXPIRE_TIME
      })
...
async def user_register(body: AuthRegisterBody) -> AuthRegisterResponse:
  ...
  if len(duplicated_fields) > 0:
    return ORJSONResponse(status_code=409, content={
      ...
      'loc': duplicated_fields
    })
  
  user = User(
    email = body.email,
    bcryptPassword = bcrypt.hashpw(body.password.encode('utf-8'), bcrypt.gensalt()).decode('utf-8'),
...
async def refresh_token(cred: JwtAuthorizationCredentials = Security(refresh_auth)) -> AuthRefreshTokenResponse:
  '''
