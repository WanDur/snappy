...
export function ContentUnavailable({
  ...
  let animationSpec: import('expo-symbols').SymbolViewProps['animationSpec'] | undefined

  if ('search' in props && props.search) {
    resolvedTitle = title ?? typeof props.search === 'string' ? `No Results for "${props.search}"` : `No Results`
    ...
    resolvedSystemImage ??= process.env.EXPO_OS === 'ios' ? 'wifi' : 'wifi.slash'
  ...
  return (
    ...
    >
      {typeof resolvedSystemImage === 'string' ? (
        <IconSymbol animationSpec={animationSpec} name={resolvedSystemImage as any} size={48} color={AC.systemGray} />
      ) : (
        resolvedSystemImage
      )}
