...
export function Segments({ defaultValue, children }: SegmentsProps) {
  ...
  return <SegmentsContext.Provider value={{ value, setValue }}>{children}</SegmentsContext.Provider>
}
...
export function SegmentsList({
  ...
  const triggers = Children.toArray(children).filter((child: any) => child.type?.displayName === 'SegmentsTrigger')
  ...
  return (
    <SegmentedControl values={labels} style={style} selectedIndex={values.indexOf(value)} onChange={handleChange} />
  )
}
