...
const FileComponent = ({ position, filePath, fileName, fileSize, fileType }: FileComponentProps) => {
  ...
  const styles = StyleSheet.create({
    fileComponentContainer: {
      ...
      shadowRadius: 5,
      elevation: 3,
      backgroundColor: chatColors.file[position].backgroundColor
    },
    fileInfo: {
      flex: 1
    },
    fileName: {
      fontWeight: '600',
      color: chatColors.file[position].color
    },
    fileType: {
      fontSize: 12,
      color: chatColors.file[position].color,
  ...
  return (
    <TouchableOpacity style={styles.fileComponentContainer} activeOpacity={0.9}>
      ...
      <View style={styles.fileInfo}>
        ...
        <Text style={styles.fileType}>{fileType.split('/')[1].toUpperCase()}</Text>
      </View>
    </TouchableOpacity>
  )
}
